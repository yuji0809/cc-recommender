...
import type { Recommendation, RawSkillEntry } from "../types/index.js";
...
function transformSkillEntry(raw: RawSkillEntry): Recommendation | null {
  try {
    ...
    const owner = repoMatch ? repoMatch[1] : raw.author;
    const repo = repoMatch ? repoMatch[2].replace(/\.git$/, "") : raw.name;
