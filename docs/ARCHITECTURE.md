...
│  │  - mcp/, plugins/, skills/           │                  │
│  │  - mcp-fetcher, official-mcp-fetcher  │                  │
│  │  - plugin-fetcher, skill-fetcher     │                  │
...
│  - remote-data.repository.ts (CDN/GitHub fetch)             │
└────────────────────┬────────────────────────────────────────┘
                     │ depends on
                     ▼
┌─────────────────────────────────────────────────────────────┐
...
**ディレクトリ・ファイル:**
- `mcp/` - MCP サーバー取得
  - `mcp-fetcher.ts` - awesome-mcp-servers から取得
  - `official-mcp-fetcher.ts` - 公式 MCP レジストリから取得
- `plugins/` - プラグイン取得
  - `plugin-fetcher.ts` - 公式マーケットプレイスから取得
- `skills/` - スキル取得
  - `skill-fetcher.ts` - awesome-claude-code から取得
  - `official-skill-fetcher.ts` - 公式スキルソースから取得
  - `github-topic-search.ts` - GitHub トピック検索（自動発見）
...
**ファイル:**
- `recommendation.repository.ts` - レコメンデーションデータベースへのアクセス（キャッシュ・ファイル/リモート切替）
- `remote-data.repository.ts` - リモートデータ取得（CDN/GitHub の JSON）

**特徴:**
- シングルトンパターンでキャッシュを実装
- ファイルシステムまたはリモート URL からの読み込み
- JSON パース・バリデーション
...
- `tests/analyzer/` - 分析機能のテスト
- `tests/fetchers/` - フェッチャーのテスト
- `tests/recommender/` - 推薦機能のテスト
