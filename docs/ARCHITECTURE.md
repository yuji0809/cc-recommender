...
**ファイル:**
- `mcp-fetcher.ts` - awesome-mcp-servers から MCP サーバー情報取得
- `official-mcp-fetcher.ts` - 公式 MCP レジストリから情報取得
- `plugin-fetcher.ts` - 公式マーケットプレイスからプラグイン情報取得
- `skill-fetcher.ts` - awesome-claude-code からスキル情報取得
